...
final class BSONToIntTransformer implements PropertyTransformerInterface, PropertyTransformerSupportInterface, PrioritizedPropertyTransformerInterface
{
    ...
    public function supports(TypesMatching $types, SourcePropertyMetadata $source, TargetPropertyMetadata $target, MapperMetadata $mapperMetadata): bool
    {
        ...
        // Transforms only to "int"
        if (Type::BUILTIN_TYPE_INT !== $types->getTargetUniqueType($sourceType)?->getBuiltinType()) {
            return false;
        }
        ...
        if (Type::BUILTIN_TYPE_INT === $sourceType->getBuiltinType()) {
            return true;
        }
